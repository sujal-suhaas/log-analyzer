...
- UTF-8 text, max 50 MB / 1M lines per file, `.log .txt .json .csv .jsonl .ndjson`.
- Select parser before importing, or change it afterward to re-parse the current file. No format auto-detection.
- **Workspace**: import as many files as you like; each stays listed with its own parser and line count. Only the active file is parsed in the Worker, so switching re-reads that file (~1 s for 50 MB) and memory stays at one dataset. A failed or cancelled import never adds a list entry, re-importing the same name replaces that entry, and switching resets search/filters plus selection.
