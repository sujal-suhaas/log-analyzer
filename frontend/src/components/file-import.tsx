  ...
  return (
    <div className="mx-auto w-full max-w-xl">
      <button type="button" disabled={disabled} aria-label="Import log file"
        ...
        <span className="text-xs text-muted-foreground">UTF-8 text · Up to 50 MB · One file at a time</span>
      </button>
      <input ref={inputRef} type="file" accept={ACCEPT} className="hidden" aria-label="Choose log file" disabled={disabled} onChange={e => { if (e.target.files) onFiles(Array.from(e.target.files)); e.target.value = '' }} />
